...
object Main extends ZIOAppDefault {
  ...
  object getRoutes extends Http4sDsl[Task] {
    val zio_routes: HttpRoutes[Task] = HttpRoutes.of {
  ...
  def run: ZIO[Any with ZIOAppArgs with Scope, Throwable, ExitCode] = entry_point.provideLayer(
    zioHttp4sLayer).catchAll(t => ZIO.succeed(t.printStackTrace()).map(_ => ExitCode.failure))
